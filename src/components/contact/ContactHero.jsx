  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const material = new THREE.MeshPhongMaterial({
        ...
        emissive: new THREE.Color().setHSL(Math.random() * 0.3 + 0.55, 0.8, 0.4),
      })
      const particle = new THREE.Mesh(geometry, material)
      particle.position.set((Math.random() - 0.5) * 12, (Math.random() - 0.5) * 12, (Math.random() - 0.5) * 5)
      particles.push({
        mesh: particle,
    ...
    const light2 = new THREE.PointLight(0x0066ff, 0.8)
    ...
    const animate = () => {
      ...
      particles.forEach((particle) => {
        ...
        if (Math.abs(particle.mesh.position.x) > 6) particle.vx *= -1
        if (Math.abs(particle.mesh.position.y) > 6) particle.vy *= -1
        if (Math.abs(particle.mesh.position.z) > 2.5) particle.vz *= -1
      })
